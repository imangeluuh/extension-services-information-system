...
class ExtensionProgram(db.Model):
    ...
    IsArchived = db.Column(db.Boolean, index=True, default=False)
...
class Achievement(db.Model):
    ...
    UserId = db.Column(db.UUID(as_uuid=True), db.ForeignKey('APMSUser.id', ondelete="CASCADE"))

    TypeOfAchievement = db.Column(db.String)
    DateOfAttainment = db.Column(db.Date)
    Description = db.Column(db.String)
    Story = db.Column(db.Text)
    LinkReference = db.Column(db.String)
    Editable = db.Column(db.Boolean, nullable=False, default=False)
    ExtensionID = db.Column(db.Integer, db.ForeignKey('ESISActivity.ActivityId', ondelete="CASCADE") )